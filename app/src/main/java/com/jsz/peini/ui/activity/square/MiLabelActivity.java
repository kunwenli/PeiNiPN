...
public class MiLabelActivity extends BaseActivity implements AdapterView.OnItemClickListener {
    ...
    public ArrayList<String> mBottomDatas;
    public ArrayList<String> mTopDatas;

    private ChannelAdapter mTopAdapter, mBottomAdapter;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case Conversion.DATA_SUCCESS:
                    SuccessfulBean obj = (SuccessfulBean) msg.obj;
                    ToastUtils.showToast(mActivity, obj.getResultDesc());
                    finish();
                    break;
                default:
                    break;
    ...
    public void initView() {
        ...
        mTopDatas = new ArrayList<String>();
        mBottomDatas = new ArrayList<String>();

        mChannelText.setText("已选标签" + mTopDatas.size() + "/3");
        initNetWork();
    }

    @Override
    public void initData() {
        mTopAdapter = new ChannelAdapter(mTopDatas, this);
        mBottomAdapter = new ChannelAdapter(mBottomDatas, this);
    ...
    private void onBottomItemClick(View view, int position) {
        if (mTopAdapter.getItem().size() > 2) {
            ToastUtils.showToast(mActivity, "最多选择三个标签");
            return;
        }
