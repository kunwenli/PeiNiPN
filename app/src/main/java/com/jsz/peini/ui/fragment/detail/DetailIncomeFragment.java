...
public class DetailIncomeFragment extends BaseFragment {
     ...
     */
    String type = "1";
    @InjectView(R.id.detail_income)
    RecyclerView mDetailIncome;
    public List<ScoreHistoryListBean.DataBean> mList;
    public DetailAdapter mAdapter;

    @Override
    public View initViews() {
        return UiUtils.inflate(mActivity, R.layout.fragment_detailincome);
    }

    @Override
    public void initData() {
        mList = new ArrayList<>();
        mDetailIncome.setLayoutManager(new LinearLayoutManager(mActivity));
        mAdapter = new DetailAdapter(mActivity, mList);
        mDetailIncome.setAdapter(mAdapter);
